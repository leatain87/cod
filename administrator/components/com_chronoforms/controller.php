...
class ChronoFormsAdmin extends JObject{
	...
	public static function admin_form(){
		...
		if(empty($formname)){
			if(empty($form_id)){
				...
				$result = $database->getTableColumns($table_name, false);
				...
				if(isset($_POST['cid']) && !empty($_POST['cid'])){
					$_POST['cf_id'] = $_POST['cid'][0];
					//load all selected records data
					$database->setQuery("SELECT * FROM ".$table_name." WHERE ".$primary." IN ('".implode("','", $_POST['cid'])."')");
		...
		if($event == 'cf_csv_export'){
			...
			$csv_action_params = new JCFParameter('');
	...
	public static function action_task(){
		...
		if(!empty($form_action)){
			...
			$actionclassname = preg_replace_callback('/(?:^|_)(.?)/', function($matches){return strtoupper($matches[1]);}, 'cfaction_'.$form_action);//preg_replace('/(?:^|_)(.?)/e', "strtoupper('$1')", 'cfaction_'.$form_action);
	...
	public static function publish($task = 'publish'){
		if(isset($_POST['cid']) && !empty($_POST['cid'])){
			...
			$database->setQuery("UPDATE #__chronoforms SET published='".$published."' WHERE id='".$_POST['cid'][0]."'");
			if(!$database->query()){
	...
	public static function edit_form(){
		...
		if(isset($_POST['cid']) && !empty($_POST['cid'])){
			$mainframe = JFactory::getApplication();
			$database = JFactory::getDBO();
			$database->setQuery("SELECT * FROM #__chronoforms WHERE id='".$_POST['cid'][0]."'");
	...
	public static function delete_temp(){
		...
		if(is_array($cids)){
			//$this->delete_form($cids);
		}
	}

	public static function delete_form($cids = array()){
		$mainframe = JFactory::getApplication();
		$database = JFactory::getDBO();
	...
	public static function copy_form(){
		...
		if(isset($_POST['cid']) && !empty($_POST['cid'])){		
			...
			$new_id = $row->id;
			unset($row);
			$query = "SELECT `id` FROM `#__chronoform_actions` WHERE `chronoform_id` = '".$_POST['cid'][0]."'";
			$database->setQuery($query);
			$row_ids = $database->loadObjectList();
	...
	public static function restore_forms(){
		...
		if(is_array($files) && !empty($files)){
			...
			if($exten[count($exten)-1] == 'cf4bak'){			
				...
				$uploadedfile = JFile::upload($file['tmp_name'], $path.DS.$filename, false, true);
				if(!$uploadedfile){
	...
	public static function show_data(){
		...
		if((isset($_POST['table_name']) && !empty($_POST['table_name'])) || (isset($_GET['table_name']) && !empty($_GET['table_name']))){
			...
			//show data
			if(isset($_POST['cid']) && !empty($_POST['cid'])){
				$database->setQuery("SELECT * FROM ".$table_name." WHERE ".$primary."='".$_POST['cid'][0]."'");
	...
	public static function _getFormFieldsNames($form_code){
		$fields_names = array();
	...
	public static function _save_form_wizard(){
		...
		if(isset($_POST['data']['Chronoform']['id']) && !empty($_POST['data']['Chronoform']['id'])){
			...
			if(isset($_POST['wizard_mode'])){// && $_POST['wizard_mode'] == 'easy'){
				...
				$form_params = new JCFParameter($form->params);
	...
	public static function _processWizardCode($formdata){
		$content = '';
		...
		foreach($formdata as $formdata_key => $formdata_element){
			...
			default:
				...
				if(file_exists(JPATH_SITE.DS."administrator".DS."components".DS."com_chronoforms".DS.'form_elements'.DS.$file_name.'.php')){
					...
					$elementclassname = preg_replace_callback('/(?:^|_)(.?)/', function($matches){return strtoupper($matches[1]);}, 'chrono_forms_'.$file_name);//preg_replace('/(?:^|_)(.?)/e', "strtoupper('$1')", 'chrono_forms_'.$file_name);
