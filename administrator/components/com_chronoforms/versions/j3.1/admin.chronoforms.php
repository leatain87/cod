...
class ChronoFormsAdminVersion extends JObject{
	...
	public static function validatelicensedata(){
		$mainframe = JFactory::getApplication();
		$uri = JFactory::getURI();
		...
		$configs = new JCFParameter($result->params);
		$postfields = array();
		...
		if(trim(JRequest::getVar('instantcode')) && $configs->get('licensecode', '')){
			...
			if(((int)$step5 < (strtotime("now") + (24 * 60 * 60)))&&((int)$step5 > (strtotime("now") - (24 * 60 * 60)))){
				...
				if($result){
					...
					$newparams = new JCFParameter($result->params);
