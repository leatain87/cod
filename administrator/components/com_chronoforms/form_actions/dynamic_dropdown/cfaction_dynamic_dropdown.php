...
class CfactionDynamicDropdownHelper{
	function load($form = null, $actiondata = null){
		...
		if(((bool)$params->get('enable_ajax', 0) === false && empty($dynamic_values)) || empty($source_id) || empty($target_id)){die($target_id);
			return false;
		}		
		...
		?>
			window.addEvent('load', function() {
				document.id('<?php echo $source_id; ?>').addEvent('change', function(){
					<?php if((bool)$params->get('enable_ajax', 0) === false): ?>
						<?php foreach($dynamic_values as $k => $dynamic_value): ?>
							if(document.id('<?php echo $source_id; ?>').get('value') == '<?php echo $k; ?>'){
								document.id('<?php echo $target_id; ?>').empty();
								<?php foreach($dynamic_value as $option_value => $option_title): ?>
									new Element('option', {'value': '<?php echo $option_value; ?>', 'text': '<?php echo $option_title; ?>'}).inject(document.id('<?php echo $target_id; ?>'));
								<?php endforeach; ?>
								document.id('<?php echo $target_id; ?>').fireEvent('change');
							}
						<?php endforeach; ?>
					<?php else: ?>
						var load_req = new Request({
							...
							onRequest: function(){
								document.id('<?php echo $target_id; ?>').empty();
								new Element('option', {'value': '', 'text': 'Loading...'}).inject(document.id('<?php echo $target_id; ?>'));
							},
							onSuccess: function(responseText){
								...
								response_data.each(function(line){
									...
									new Element('option', {'value': line_data[0], 'text': line_data[1]}).inject(document.id('<?php echo $target_id; ?>'));
								});
								document.id('<?php echo $target_id; ?>').fireEvent('change');
							},
							onFailure: function(){
								document.id('<?php echo $target_id; ?>').empty();
								new Element('option', {'value': '', 'text': 'Loading failed.'}).inject(document.id('<?php echo $target_id; ?>'));
							}
						});
						load_req.send(document.id('<?php echo $source_id; ?>').get('name')+'='+document.id('<?php echo $source_id; ?>').get('value'));
					<?php endif; ?>
				});
			});
		<?php
