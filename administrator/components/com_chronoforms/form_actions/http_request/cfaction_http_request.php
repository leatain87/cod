	...
	function loadAction($form, $actiondata){
		...
		//<![CDATA[
			var request_caller_<?php echo $actiondata->id; ?> = function(){
				var myForm = document.id('chronoform_<?php echo $form->form_name; ?>');
				if(myForm.hasClass('hasValidation') == <?php echo ($params->get('request_event', 'submit') == 'submit') ? "false" : "true"; ?>){
					myForm.set('send', {
						...
						onSuccess: function(responseText, responseXML){
							...
							?>
								document.id('<?php echo trim($params->get('response_element_id', '')); ?>').set('html', responseText);
							<?php
								endif;
							?>
							<?php
			...
			window.addEvent('domready', function() {
				...
				?>
					if(document.id('chronoform_<?php echo $form->form_name; ?>').hasClass('hasValidation') == false){
						document.id('chronoform_<?php echo $form->form_name; ?>').addEvent('submit', request_caller_<?php echo $actiondata->id; ?>);
					}
				<?php
					else:
				?>
					document.id('<?php echo $params->get('event_element_id', ''); ?>').addEvent('<?php echo $params->get('request_event', 'submit'); ?>', request_caller_<?php echo $actiondata->id; ?>);
				<?php
					endif;
				?>
			});
