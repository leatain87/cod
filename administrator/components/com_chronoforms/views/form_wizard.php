...
<script language="javascript" type="text/javascript">
	<?php
	$jversion = new JVersion();
	if($jversion->RELEASE >= 3.2 AND false):
	?>
          ...
          <!--<div class="half first_half">-->
			...
			<?php
				...
				foreach($form_elements as $k => $form_element){
					...
					$elementclassname = preg_replace_callback('/(?:^|_)(.?)/', function($matches){return strtoupper($matches[1]);}, 'chrono_forms_'.$form_element);//preg_replace('/(?:^|_)(.?)/e', "strtoupper('$1')", 'chrono_forms_'.$form_element);
		...
		<div id="widgets_elements" class="elements_list elements_accordion_pane">
			...
			<?php foreach($form_widgets as $k => $form_widget): ?>
			    <?php
					...
					$elementclassname = preg_replace_callback('/(?:^|_)(.?)/', function($matches){return strtoupper($matches[1]);}, 'chrono_forms_input_'.$form_widget);//preg_replace('/(?:^|_)(.?)/e', "strtoupper('$1')", 'chrono_forms_input_'.$form_widget);
					if(class_exists($elementclassname)){
						$elementclass = new $elementclassname;
						$element_params = $elementclass->load(true);
					}else{
...
<div class="ccms_box" id="drop_box" style="float:right; width:56%;">
  <form action="index.php" method="post" name="adminForm" id="adminForm" id="adminForm">
					...
					<?php if(!empty($form->wizardcode)): ?>				
						...
						<?php foreach($wizardcode as $formdata_key => $formdata_element): ?>
							...
							if(file_exists(JPATH_SITE.DS."administrator".DS."components".DS."com_chronoforms".DS.'form_elements'.DS.$form_element.'.php')){									
								...
								$elementclassname = preg_replace_callback('/(?:^|_)(.?)/', function($matches){return strtoupper($matches[1]);}, 'chrono_forms_input_'.$form_element);//preg_replace('/(?:^|_)(.?)/e', "strtoupper('$1')", 'chrono_forms_input_'.$form_element);
								if(class_exists($elementclassname)){
									$elementclass = new $elementclassname;
									$element_params = $elementclass->load(true);
								}else{
	...
	<?php echo $DropTabsHelper->tabStart('settings'); ?>
		<?php
			if(!empty($form)){
				$params = new JCFParameter($form->params);
			}else{
...
function loadActionFile($action_data, $action_index){
	...
	if(file_exists($action_file1)){
		require_once($action_file1);
		$actionclassname = preg_replace_callback('/(?:^|_)(.?)/', function($matches){return strtoupper($matches[1]);}, 'cfaction_'.$action_data->type);//preg_replace('/(?:^|_)(.?)/e', "strtoupper('$1')", 'cfaction_'.$action_data->type);
		if(class_exists($actionclassname)){
			$actionclass = new $actionclassname;
			$action_params = $actionclass->load(true);
		}
	...
	if(file_exists($filename)){
		...
		if(!is_array($action_data->params)){
			$aparams = new JCFParameter($action_data->params);
...
function loadActionParams($action_name){
	...
	if(file_exists($action_file1)){
		require_once($action_file1);
		$actionclassname = preg_replace_callback('/(?:^|_)(.?)/', function($matches){return strtoupper($matches[1]);}, 'cfaction_'.$action_name);//preg_replace('/(?:^|_)(.?)/e', "strtoupper('$1')", 'cfaction_'.$action_name);
		if(class_exists($actionclassname)){
			$actionclass = new $actionclassname;
			$action_params = $actionclass->load(true);
		}
